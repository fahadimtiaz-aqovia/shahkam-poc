...
function capacityModule(){
  ...
  if(st===0){
    ...
    body=capacityScenarioTable()
      + `<div class="grid-2"><div class="card panel"><div class="panel-head"><div><h3>Capacity forecast</h3><p>Backlog trajectory with a 95% band — where load is heading.</p></div>${howBtn('forecast','How?')}</div><div class="chart-wrap"><canvas id="capFc"></canvas></div></div>
      <div class="card panel"><h3>Plan at a glance</h3><p>Delivery probability by scenario — the recommended plan clears the 90% target.</p><div class="scenario-cards"><div class="card scenario-card"><h4>A · CURRENT</h4><div class="score">61%</div><span class="risk-pill risk-high">AT RISK</span></div><div class="card scenario-card"><h4>C · REALLOCATE</h4><div class="score">84%</div><span class="risk-pill risk-med">BETTER</span></div><div class="card scenario-card recommended"><span class="label">Recommended</span><h4>D · COMBINED</h4><div class="score">91%</div><span class="risk-pill risk-low">ON TRACK</span></div></div></div></div>`;
